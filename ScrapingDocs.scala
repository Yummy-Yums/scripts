...
val links = indexDoc.select("h2#specifications").nextAll.select("div.index a").asScala
...
val articles = for ((url, tooltip, name) <- linkData) yield {
  ...
  val summary = doc.select("article.main-page-content > div > p").asScala.headOption match {
